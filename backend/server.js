...
const io = new Server(server, {
  cors: {
    origin: "http://127.0.0.1:5000", // The frontend origin
    methods: ["GET", "POST"],
...
// Home route
app.get('/', (req, res) => {
  const file = res.sendFile(path.join(__dirname, '../tic-tac-toe-frontend', 'login-signup.html'));// Serve the login-signup.html file
});
...
app.post('/game', authenticateToken, (req, res) => {
  const file = res.sendFile(path.join(__dirname, '../tic-tac-toe-frontend', 'game.html'));
});

app.post('/home', authenticateToken, (req, res) => {
  const file = res.sendFile(path.join(__dirname, '../tic-tac-toe-frontend', 'home.html'));
});
...
app.get('/history', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'An error occurred while fetching game history.' });
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('joinQueue', (username) => {
    ...
    if (waitingPlayers.length >= 2) {
      ...
      const stmt = db.prepare('INSERT INTO games (gameId, cross, circle, moves, result) VALUES (?, ?, ?, ?, ?)');
      stmt.run(gameId, player1.username, player2.username, JSON.stringify(gameData.moves), gameData.result, function (err) {
        if (err) {
  ...
  socket.on('joinGame', ({room, p1, p2, p1Id, p2Id}) => {
    ...
    socket.on('makeMove', async (gameId, move, currentTurn) => {
      try {
          ...
          socket.emit('error', 'An unexpected error occurred while processing your move.');
      }
  });  
});
//waitingqueue
